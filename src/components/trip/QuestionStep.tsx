...
export default function QuestionStep({
  ...
  function selectedCount(q: TripQuestion) {
  ...
  const ready =
    ...
    stepQuestions.every((q) => !q.required || selectedCount(q) >= q.minSelections);

  function toggle(q: TripQuestion, answerId: string) {
    const multiple = q.type === "MULTIPLE_CHOICE";
    setAnswers((prev) => {
      if (!multiple) return { ...prev, [q.id]: answerId };

      const current = prev[q.id];
      const currentArr = Array.isArray(current) ? current : current ? [current] : [];
      if (currentArr.includes(answerId)) {
        return { ...prev, [q.id]: currentArr.filter((id) => id !== answerId) };
      }
      if (currentArr.length >= q.maxSelections) return prev;
      return { ...prev, [q.id]: [...currentArr, answerId] };
    });
  }

  function handleNext() {
    if (!ready) return;
    const merged = { ...readDraft().answers };
    for (const q of stepQuestions) {
      if (selectedCount(q) > 0) merged[q.id] = answers[q.id];
    }
    writeDraft({ answers: merged });
